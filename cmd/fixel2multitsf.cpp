...
void usage ()
{
  ...
  + Argument ("tsf_par",      "the output track scalar file").type_file_out ();


  OPTIONS
  + Option ("tsf_perp", "optional output track scalar file for perpendicular fixel values")
...
void run ()
{
  ...
  DWI::Tractography::ScalarWriter<float> tsf_writer_par (argument[2], properties);
  ...
  ProgressBar progress ("mapping the fixel values to streamline points", num_tracks);
  DWI::Tractography::Streamline<float> tck;
  DWI::Tractography::TrackScalar<float> scalars;
  DWI::Tractography::TrackScalar<float> scalars_perp; // For perpendicular fixels
  ...
  while (reader (tck)) {
    ...
    scalars_perp = scalars; // Initialize perpendicular scalars
    for (size_t p = 0; p < tck.size(); ++p) {
      ...
      for (SetVoxelDir::const_iterator d = dixels.begin(); d != dixels.end(); ++d) {
        if ((int)round(voxel_pos[0]) == (*d)[0] && (int)round(voxel_pos[1]) == (*d)[1] && (int)round(voxel_pos[2]) == (*d)[2]) {
          ...
          float smallest_dp = 1.0f; // dot product ranges from 0 to 1 for normalized vectors
          int32_t most_perpendicular_fixel_index = -1;
          ...
          for (size_t fixel = 0; fixel < num_fixels_in_voxel; ++fixel) {
            ...
            if (dp < smallest_dp) {
              smallest_dp = dp;
              most_perpendicular_fixel_index = fixel;
            }
          }
          ...
          // If the largest dot product is below the threshold, we can still assign the most perpendicular fixel
          float perpendicular_fixel_value = 0.0f;
          if (most_perpendicular_fixel_index >= 0) {
            in_data_image.index(0) = offset + most_perpendicular_fixel_index;
            perpendicular_fixel_value = in_data_image.value();
            scalars_perp[p] = perpendicular_fixel_value;
          }
          break;
        }
      }
    }
    tsf_writer_par (scalars);
    if (tsf_writer_perp) {
      (*tsf_writer_perp) (scalars_perp);
    }
    progress++;
  }
}
