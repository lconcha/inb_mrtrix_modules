...
void usage ()
{
  ...
  + Option ("perp", "output TSF file for most perpendicular fixel").argument ("filename").type_file_out ()
  + Option ("avg", "output TSF file for average of all except most parallel fixel").argument ("filename").type_file_out ();
}
...
void run ()
{
  ...
  std::unique_ptr<DWI::Tractography::ScalarWriter<float>> tsf_writer_avg;
  if (has_option("perp"))
    tsf_writer_perp.reset(new DWI::Tractography::ScalarWriter<float>(get_option_value("perp"), properties));
  if (has_option("avg"))
    tsf_writer_avg.reset(new DWI::Tractography::ScalarWriter<float>(get_option_value("avg"), properties));
  ...
  while (reader (tck)) {
    ...
    if (tsf_writer_avg) (*tsf_writer_avg)(scalars_avg);
    progress++;
  }
}